...
var getLineNavigatorClass = function() {
  function LineNavigator (file, options) {
    ...
    self.countTotalLines = function(callback) {
      return self.countLines(0, 0, callback);
    };

    self.countLines = function(offset, linesAcc, callback) {
      wrapper.readChunk(offset, chunkSize, function readChunkCallback(err, buffer, bytesRead) {
        ...
        if (chunkContent === undefined) {
          ...
          if (bytesRead > 0) {
            if (throwOnLongLines) {
              return callback('Line ' + index + ' is longer than chunk size (' + chunkSize + ')', null);
            } else {
              chunkContent = {
                lines: 1,
                length: bytesRead - 1,
            ...
            return callback('Line ' + index + ' is out of index.', null);
          }
        }

        let newLinesAcc = linesAcc + chunkContent.lines;
        if (isEof) {
          callback(null, newLinesAcc);
        } else {
          let newOffset = offset + chunkContent.length + 1;
          console.log(`Counted ${newLinesAcc} lines, offset: ${newOffset}, chunk:`, chunkContent);
          self.countLines(newOffset, newLinesAcc, callback);
        }
      });
    };
